    ...
    fetch(
      ...
      .then((res) => {
        ...
        if (res.meals) {
          ...
          $("#mealCardsSection").prepend(
            "<div id='errorMessageContainer' style='display:flex;'> <p id='errorMessageText'>No recipes match the search term '" +
              user_search_term +
              "'</p> <a id='errorMessageBtn' class='button' href='#landing' title='Search again' >Search again</a> </div>"
          );
        }
      })
      .catch((e) => console.warn(e));
...
const createMeal = (meal, type) => {
  ...
  if (meal.strTags) {
    mealMetadata += `<span>Tags:</span> ${meal.strTags
      .split(",")
      .join(", ")} <br/>`;
  }
  ...
  if (ingredients.length > 0) {
    mealMetadata += `<span>Ingredients:</span> <br/> <ul>${ingredients.join(
      ""
    )}</ul>`;
  }
...
const createMealCards = (meals) => {
  ...
  meals.forEach((meal) => {
    ...
    mealCards += `<div class="four columns"><div class="card">
